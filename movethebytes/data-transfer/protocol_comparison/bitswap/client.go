...
func (c *Client) getBlockFromPeer(ctx context.Context, cids []cid.Cid) ([]<-chan response, error) {
	...
	var (
		wants = c.runenv.R().Counter("msg.wants")
		msgs  = c.runenv.R().Counter("msg.count")
	)
	wants.Inc(int64(len(cids)))
	msgs.Inc(1)
...
func NewClient(host host.Host, runenv *runtime.RunEnv, p peer.ID, re log2.StandardLogger) *Client {
	c := &Client{
		Host:   host,
		wants:  map[string]map[string]chan response{},
		peer:   p,
