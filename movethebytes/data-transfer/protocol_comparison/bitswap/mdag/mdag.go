...
import (
	...
	//bserv "github.com/ipfs/test-plans/data-transfer/bitswap/bservice"
)
...
type SessionMaker interface {
	Session(context.Context) format.NodeGetter
}

// NewSession returns a session backed NodeGetter if the given NodeGetter
// implements SessionMaker.
func NewSession(ctx context.Context, g format.NodeGetter) format.NodeGetter {
...
func parallelWalkDepth(ctx context.Context, getLinks GetLinks, root cid.Cid, visit func(cid.Cid, int) bool, options *walkOptions) error {
	...
	for {
		...
		case <-t.C:
			Logger.Infof("InProgress :%d, TodoQ: %+v", inProgress, todoQueue)
		case <-done:
			inProgress--
